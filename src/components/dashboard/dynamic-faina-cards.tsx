...
export function DynamicFainaCards({ scrapedData, selectedShift = 'live' }: DynamicFainaCardsProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      // 2. Verificar Auth e Sincronizar
      const { data: { session } } = await supabase.auth.getSession();
      setUser(session?.user ?? null);

      if (session?.user) {
        const [prefsRes, historyRes] = await Promise.all([
          supabase.from('user_preferences').select('*').eq('user_id', session.user.id),
          supabase.from('ponteiro_history').select('*').eq('user_id', session.user.id).order('created_at', { ascending: false }).limit(500)
        ]);

        if (prefsRes.data) {
          setPreferences(prefsRes.data);
          localStorage.setItem('faina_preferences', JSON.stringify(prefsRes.data));
        }
        if (historyRes.data) {
          const formattedHistory = historyRes.data.map((h: any) => ({
            id: `${h.funcao}_${h.data_turno}`,
            funcao: h.funcao,
            sinal: h.sinal,
            original1: h.original1,
            temporario1: h.temporario1,
            original2: h.original2,
            temporario2: h.temporario2,
            dataTurno: h.data_turno,
            createdAt: h.created_at
          }));
          setHistoryData(formattedHistory);
          localStorage.setItem('ponteiro_history', JSON.stringify(formattedHistory));
        }
      }
      setIsLoading(false);
    };
    ...
    window.addEventListener('faina_preferences_updated', handleLocalUpdate);
    window.addEventListener('ponteiro_history_updated', handleLocalUpdate);
    
    return () => {
      subscription.unsubscribe();
      window.removeEventListener('faina_preferences_updated', handleLocalUpdate);
      window.removeEventListener('ponteiro_history_updated', handleLocalUpdate);
    };
  }, []);
