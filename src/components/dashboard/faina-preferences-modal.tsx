...
export function FainaPreferencesModal({ availableFainas, trigger }: FainaPreferencesModalProps) {
  ...
  useEffect(() => {
    if (user && settings?.fainas_favoritas) {
      setPreferences(settings.fainas_favoritas);
    } else {
  ...
  const savePreferences = async (updatedPrefs: any[]) => {
    if (user) {
      await updateSettings({ fainas_favoritas: updatedPrefs });
    } else {
  ...
  const handleAdd = async () => {
    ...
    toast({
      ...
      description: `A faina ${normalizedNewFaina} foi processada ${user ? 'na sua conta' : 'localmente'}.`,
    });
  };
  ...
  const handleDelete = async () => {
    ...
    toast({
      title: "Removido",
      description: `Faina removida ${user ? 'da sua conta' : 'da lista local'}.`,
    });
  };
          ...
          <div className="p-6 pb-2">
            <DialogHeader>
              <DialogTitle className="text-xl font-black uppercase tracking-tight">
                {editingId ? "Editar Configuração" : user ? `Minhas Fainas (${user.name})` : "Minhas Fainas (Local)"}
              </DialogTitle>
              <DialogDescription className="text-xs text-muted-foreground mt-1 uppercase font-bold opacity-70">
                {user ? "Dados salvos na sua conta Supabase." : "Os dados são salvos apenas neste navegador."} Teto em '0' ignora lógica circular.
              </DialogDescription>
            </DialogHeader>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 space-y-6 pb-6 mt-4">
            ...
            <div className="space-y-4">
              <h4 className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">
                {user ? "Lista Sincronizada" : "Minha Lista Local"}
              </h4>
              <div className="space-y-3">
                {preferences.length === 0 && (
                  <p className="text-[10px] italic text-muted-foreground text-center py-4">
                    {user ? "Você ainda não favoritou nenhuma faina." : "Nenhuma faina salva localmente."}
                  </p>
                )}
