...
export function FainaPreferencesModal({ availableFainas, trigger }: FainaPreferencesModalProps) {
  ...
  const syncWithSupabase = async (userId: string) => {
    ...
    try {
      ...
      if (data) {
        setPreferences(data);
        localStorage.setItem('faina_preferences', JSON.stringify(data));
  ...
  const handleAdd = async () => {
    ...
    if (user) {
      ...
      // Recarregar dados do Supabase para garantir sincronia
    ...
    toast({
      ...
      description: `A faina ${normalizedNewFaina} foi processada ${user ? 'na nuvem' : 'localmente'}.`,
    });
  };
  ...
  const handleDelete = async () => {
    ...
    toast({
      title: "Removido",
      description: "Faina removida com sucesso.",
    });
  };
            ...
            <DialogHeader>
              <div className="flex items-center justify-between mb-2">
                <DialogTitle className="text-xl font-black uppercase tracking-tight">
                  {editingId ? "Editar Faina" : "Minhas Fainas"}
                </DialogTitle>
                  ...
                  {user ? (
                    <div className="flex items-center gap-2">
                      <div className="flex flex-col items-end">
                        <span className="text-[8px] font-black text-green-500 uppercase">Sincronizado</span>
                        <span className="text-[7px] text-muted-foreground truncate max-w-[80px]">{user.email}</span>
                      </div>
                    ...
                    <Button 
                      ...
                      size="sm" 
                      onClick={handleLogin}
                      className="h-7 text-[9px] font-black uppercase border-primary/20 text-primary hover:bg-primary/10"
                    >
              ...
              <DialogDescription className="text-xs text-muted-foreground mt-1 uppercase font-bold opacity-70">
                {user ? "Dados salvos na sua conta Supabase." : "Dados salvos localmente. Entre para sincronizar."}
              </DialogDescription>
            </DialogHeader>
          </div>
